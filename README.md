...
5. Unless a fatal error occurs, in the end, the `run()` method calls `write_output()` method, which writes to an appropriately named file either in the `output` sub-directory (web app invocation) or as stipulated via the `-o` parameter to `fups.php` (manual commandline invocation), and then `fups.php` (and thus the PHP process running it) exits.
